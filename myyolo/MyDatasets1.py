...
class MyDataset(Dataset):#做数据类
    ...
    def __getitem__(self, index):#用__getitem__方法一个一个取出数据
        ...
        # #images/01.jpg 2 163 218 237 251 	 1 290 205 170 268  传入的标签是这样的，将其制作成置信度+4个偏移量+分类 对应网络输出的通道数，好做loss
        ...
        for feature_size, anchors in cfg.ANCHORS_GROUP.items():#循环标签框，并将三种建议狂，和
            ...
            for box in boxes:#循环框的个数
                # 一个gt框对应9个框，不同的尺度格子索引不一样，同一尺度下格子索引一样，但框不一样，框偏移量不一样
                cls, cx, cy, w, h = box#将每个框的数据组成的列表解包赋值给对应变量
                # 中心点偏移的计算 cx/416/13--->cx/32  cx/416/26--->cx/16
                cx_offset, cx_index = math.modf(cx * feature_size / cfg.IMG_WIDTH)#计算中心点的在每个格子x方向的偏移量和索引,
                # 中心点x的坐标乘以特征图的大小再除以原图大小，整数部分作为索引，小数部分作为偏移量，原本是cx/(图片总的大小/尺度大小）
                ...
                for i, anchor in enumerate(anchors):#循环3种建议框，并带索引分别赋值给i和anchor
                    anchor_area = cfg.ANCHORS_GROUP_AREA[feature_size][i]#每个建议框的面积，准备和实际框做iou
                    # wh偏移量的计算 是这样设计的，没有为什么  gt框/建议框
                    ...
                    # print(p_h)
                    p_area = w * h#实际标签框的面积
                    # 同一尺度下格子索引一样，就是中心点一样，框不一样，计算建议框和实际框的iou，用最小面积除以最大面积  因为只知道大小，不知道位置坐标，不能按照之前的iou方法算
                    iou = min(p_area, anchor_area) / max(p_area, anchor_area)
                    # 中心点偏移量的公式里面有格子的索引，比如说在13*13格子上的哪个地方
                    #                    [feature_size, feature_size, 3] i 就是每个尺度有3个形状，3个13*13中的哪一个
                    ...
                    # *号放在one_hot_list前面，就表示将one_hot_list中的每个元素解包成单独的参数

        return labels[13], labels[26], labels[52], img_data#13 13 3 9 26 26 3 9 52 52 3 9
