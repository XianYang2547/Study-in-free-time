...
class Trainner:#定义训练类
    def __init__(self):
        ...
        self.trainloader=DataLoader(self.traindata,batch_size=3,shuffle=True)#加载数据集
    ...
    def loss_fn(self,output,target,alpha):#定义损失函数，并传入三个参数，网络输出的数据，标签，和用来平衡正负样本损失侧重那个的权重
        ...
        loss_obj_cls=self.cls_loss_fn(output_obj[:,5:],torch.argmax(target_obj[:,5:],dim=1))#标签做argmax，因为制作的时候做了onehot，
        # 这里做argnax，传到交叉熵里面，交叉熵自带obehot，不然的话是做2次onehot
