...
class Detector(torch.nn.Module):#定义侦测模块
    ...
    def __init__(self,save_path):
        ...
        self.net = MainNet().to(device)#实例化网络
    ...
    def _filter(self, output, thresh):#赛选过滤自信度函数，将自信度合格留下来
        ...
        output = output.reshape(output.size(0), output.size(1), output.size(2), 3, -1)  # Out[1]: torch.Size([1, 13, 13, 3, 9])
        mask = output[..., 0] > thresh#获取输出自信度大于自信度阀值的目标值的掩码（即布尔值），Out[12]: torch.Size([1, 13, 13, 3])
        idxs = mask.nonzero()#将索引取出来其形状（N,H,W,3） 得到mask里面true的位置索引
    ...
    def _parse(self, idxs, vecs, t, anchors):#定义解析函数，并给4个参数分别是上面筛选合格的框的索引，9个值（中心点偏移和框的偏移即类别数），
        ...
        # idxs有框的信息，比如说是三个类型框的哪一个，用于反算索引建议框的wh
        anchors = torch.Tensor(anchors).to(device)#将建议框转为Tensor类型
        # idxs是 6 5 是13*13格子的索引 0 1 2 是3种框的索引
        # tensor([[0, 6, 5, 1],
        #         [0, 6, 5, 2],
        #         [0, 6, 6, 1],
        #         [0, 6, 6, 2]], device='cuda:0')
        a = idxs[:, 3]#表示拿到3个框对应的索引，有3个不同类型的框嘛 1 2 1 2
        ...
        # [[360, 360], [360, 180], [180, 360]]  anchors[a, 0]就是取anchors[1,0位置的和2,0 ,1,0, 2,0] a=[1 2 1 2]  取出来为[360., 180., 360., 180.]
...
if __name__ == '__main__':
    ...
    for image_file in img_name_list:
        ...
        out_value = detector(img, 0.7, cfg.ANCHORS_GROUP)#调用侦测函数并将数据，自信度阀值和建议框传入
        boxes = []#定义空列表用来装框
