...
class REBNCONV(nn.Module):
    """conv+bn+relu
    在 U2Net 模型中，REBNCONV 模块被多次使用，作为编码器和解码器中的基本组件，
    用于构建整个 U2Net 网络的层次结构。
    """

    def __init__(self, in_ch=3, out_ch=3, dirate=1):
        super(REBNCONV, self).__init__()
        # padding和dilation一样时，尺寸不变
...
class RSU7(nn.Module):  # UNet07DRES(nn.Module):
    """中间通道数为 12 是在 RSU7 组件中经过实验或经验选择得到的一个超参数值，\
    用于实现 U2Net 模型的图像编码和解码过程。"""
...
class U2NET(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.outconv = nn.Conv2d(in_channels=6, out_channels=out_ch, kernel_size=1)
